# Insumo-Producto para cuentas satélite
...
# Para acceder a cualquier elemento de nuestra matriz, utilizamos "índices"
M[2, 3]
M[2, 2]
...
# La sustracción se define como la resta de elementos en posiciones
# correspon dientes. Solo las matrices de exactamente las mismas
# dimensiones pueden ser restadas.
...
# Las matrices son iguales si tienen las mismas dimensiones
# si los elementos en posiciones correspondientes son iguales.
...
# Utilizando el primer método que usamos para crear una matriz,
# en vez de pasarle un vector a la función, podemos pasarle un
# solo número.
...
# Multiplicación de una matriz por un solo número (un escalar).
# Cada elemento de la matriz se multiplica por ese valor.
# ¿Ejemplo de para qué sirve esto?
...
# Para que sean conformables para multiplicación,
# el número de columnas en la matriz de la izquierda debe ser
# igual al número de filas en la matriz de la
# derecha.

# Esta definición también incluye el hecho que el producto de
# las dos matrices "P" tendrá el mismo número de filas que "M"
# y el mismo número de columnas que "Q".
...
# En álgebra matricial, el orden de los factores sí importa.
# En este ejemplo, por la regla de filas y columnas el producto
# de Q x M no puede ser calculado pues hay tres columnas en Q
# pero solo dos filas en M.
...
# Esta operación no tiene paralelos en el álgebra matricial,
# pero es importante en ciertas operaciones de insumo-producto.
# M' (M prima) o M^t (M transpuesta) es una matriz n x m en que
# la fila i de M se convierte en la columna i de M^t

Mt <- t(M)
t(M)

# En álgebra matricial, la notación exige que un vector
# "horizontal" se denomine como un vector transpuesto o "prima",
# pues ha sido manipulado para po der ser multiplicado con una
# matriz que tiene ciertos requerimientos de número de columnas
# o filas.

# En R, esto es irrelevante, pues los vectores son agnósticos
# con respecto a las dimensiones y se pueden utilizar de manera
# indistinta en pre o posmultiplicación, siempre y cuando tenga
# el largo adecuado respecto de las dimensiones de la matriz con
# la que se quiere operar. R hará el trabajo. Incluso es posible
# usar una multiplicación simple para multiplicar por el vector,
# entendiendo la regla del reciclaje.

# Al usar la función de transposición, R transforma un vector
# en matriz.
t(c)
t(t(c))
...
# Por la forma en que definimos la multiplicación de matrices
# y la igualdad de matrices, el sistema de ecuaciones puede
# ser representado:

#               Ax = b

# ¿Cómo encontramos la respuesta a las incógnitas?
...
# La división en álgebra matricial no está definida y
# no podemos usar % / %

# Pero en álgebra ordinario también se puede multiplicar
# ambos lados por el recíproco de 3, que es (1/3) o 3^(-1),
# así que:
...
# Pueden recordar el proceso tedioso de encontrar la
# determinante de una matriz 2 x 2 sustrayendo los productos
# cruzados por algún método. Nos ahorraremos este cálculo manual
# y dejaremos que la computadora lo haga
